    ...
    <header id="page-top" class="blog-banner">
        <div class="container" id="blog">
            <div class="row blog-header text-center wow fadeInUp">
                <div class="col-sm-12">
                    <h3>{{ $user->surname . ', ' . $user->name . "'s profile" }}</h3>
                    <h4><a href="{{ url('') }}"> Home </a> &gt; View profile </h4>
                </div>
            </div>
        </div>
    </header>
                    ...
                    <aside class="widget wiget-recent-post">
                        ...
                        @else
                            @foreach($tweets as $tweet)
                                @if($tweet->visible || (isset($tweet->belongs_to_logged_in_user) and $tweet->belongs_to_logged_in_user))
                                    <div class="recent-post-box">
                                        <div class="recent-title">
                                            <a>{{ $tweet->text }}</a>
